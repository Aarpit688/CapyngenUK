...
const NavbarTest = () => {
  ...
  const [persistDropdown, setPersistDropdown] = useState(false); // <-- new: persist when opened by click
  ...
  useEffect(() => {
    ...
    // only auto-close on hover-out when not persisted (clicked-open)
    if (!isNavItemHovered && !isDropdownHovered && !persistDropdown) {
      // small delay so pointer can move between elements without closing
  ...
  // outside click listener: close dropdown if click outside navbar/dropdown
  useEffect(() => {
    const handleDocumentClick = (e) => {
      const target = e.target;
      // if click is inside navRef or inside dropdownRef, ignore
      if (
  ...
  const renderDropdown = (type) => {
    ...
    return (
      <div
        ref={dropdownRef} // <-- track the currently rendered dropdown
        ...
        <nav className="hidden lg:flex items-center space-x-8 text-sm font-medium text-white relative">
          {navItems.map((item) => (
            ...
            >
              ...
              {/* dropdown rendered relative to this wrapper; top-full ensures it's directly under the nav link */}
              {item.dropdown &&
